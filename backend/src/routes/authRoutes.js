...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (userRole === 'student') {
        if (!usn || !usn.trim()) return res.status(400).json({ message: 'USN is required for students' });
        
        const usnUpper = usn.trim().toUpperCase();
        const userCheck = await pool.query('SELECT * FROM student WHERE email = $1 OR usn = $2', [email, usnUpper]);
        ...
        try {
          result = await pool.query(
            ...
            [name, email, passwordHash, usnUpper, 'student']
          );
        } catch (insertErr) {
          // If that fails, check if it's a column issue
          if (insertErr.code === '42703' || insertErr.message.includes('column')) {
            throw new Error(`Database schema mismatch: ${insertErr.message}. Please check your student table columns.`);
          }
          throw insertErr;
        }
        ...
        try {
          ...
          if (insertErr.code === '42703' || insertErr.message.includes('column')) {
            throw new Error(`Database schema mismatch: ${insertErr.message}. Please check your faculty table columns.`);
          }
          throw insertErr;
        }
        ...
        try {
          ...
          if (insertErr.code === '42703' || insertErr.message.includes('column')) {
            throw new Error(`Database schema mismatch: ${insertErr.message}. Please check your admin table columns.`);
          }
          throw insertErr;
        }
