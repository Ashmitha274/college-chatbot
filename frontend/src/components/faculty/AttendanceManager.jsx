    ...
    <section className="rounded-xl bg-white p-6 shadow">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((row, idx) => {
              const selectedStudent = students.find(s => s.student_id === parseInt(row.student_id));
              return (
                <tr key={idx} className="border-b border-slate-100">
                  ...
                  <td className="px-3 py-2">{selectedStudent?.usn || 'N/A'}</td>
                  <td className="px-3 py-2">{selectedStudent?.email || 'N/A'}</td>
                  <td className="px-3 py-2">{selectedStudent?.phone || 'N/A'}</td>
                  <td className="px-3 py-2">
                    <select
                      className="w-full rounded border border-slate-200 px-2 py-1"
                      value={row.status}
